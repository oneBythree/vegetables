<template>
    <div>
        <!-- 头部 -->
        <v-header :header-title="haedertitle" :right-icon="rightIcon" @right-action="addAction" @left-action="goBack" :is-right-text="isRightText">
        </v-header>
        <div class="m-content">
            <!-- 搜索tab选项卡 start-->
            <nav-bar :items="items" :selected.sync="selected"></nav-bar>
            <!-- 搜索tab选项卡 end-->
            <drop-select :type-name="typeName" :my-drop-name="myDropName1" :header-title="headerTitle1" :drop-name.sync="selected" :droplists="droplists" @drop-select-item="dropSelectItem" :select-droped.sync="isSelectDrop1" :transition="transitionDrop">
            ...
            <drop-select :type-name="typeName" :my-drop-name="myDropName3" :header-title="headerTitle2" :drop-name.sync="selected" :select-droped.sync="isSelectDrop3" :transition="transitionDrop">
            ...
            <div class="veges-list-wapper" :style="{height:vegesHeight}" v-if="!isLoading" transition="down">
                    ...
                    <dd class="g-table-fix">
                        <div class="g-cell">
                            <p class="webkit-clamp1">
                                <strong>商品</strong>
                                <span>{{come.MEAT_NAME}}</span>
                            </p>
                        </div>
                        <div class="g-cell">
                            <strong>日期</strong>
                            <span>{{come.IN_DATE.split(' ')[0]}}</span>
                        </div>
                    </dd>
                    <dd class="g-table-fix">
                        <div class="g-cell">
        ...
        <v-loading :is-loading.sync="isLoading"></v-loading>
    </div>
</template>
<script>
...
const listUrl = 'mobileTrace.do?method=getSaleMeatIn';
        ...
        methods: {
            ...
            dateTimer: function(type) {
                ...
                if (type == 'start') {
                    ...
                    picker2 = new mui.DtPicker(param);
                ...
                $.ajax({
                    ...
                    'success': function(data) {
                        ...
                        if (oldData.result) {
                            ...
                            that.isNOData = !that.comes.length > 0 ? true : false;
            ...
            getCustomerData: function() { //获取客户数据
                var that = this;
                var url = customerUrl + '&node_id=' + this.nodeId + '&type=2004&retailer=';
                $.ajax({
                    type: "GET",
                    url: url,
                    dataType: "json",
