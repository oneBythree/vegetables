<template>
    <div>
        <!-- 头部 -->
        <v-header :header-title="haedertitle" @left-action="goBack">
        </v-header>
        <div class="m-content">
            <!-- 搜索tab选项卡 start-->
            ...
            <div class="veges-list-wapper" :style="{height:vegesHeight}" v-if="!isLoading">
        ...
        <v-loading :is-loading.sync="isLoading" :is-before="isBefore"></v-loading>
    </div>
</template>
<script>
...
const listUrl = 'mobileTrace.do?method=getSaleMeatOut';
    ...
    data() {
        ...
        methods: {
            ...
            customerSelectedFuc: function(item) { //选择客户点击事件
                ...
                this.items[0].text = item.index == -1 ? '客户筛选' : item.value;
                this.selected = null;
                this.page = 1; // 重置页码数
                this.comes = [];
                // this.isLoading = true;
